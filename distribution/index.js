...
function getAttributes(template, reg) {
  var templateSane = template.replace(/\t|\n/g, '');
  var tags = [];
...
function getChildrenTags(tagsArray, attributes) {
  ...
  for (var i = 0; i < tempArray.length; i++) {
    ...
    var indexFinish = arrayMayorThanI.indexOf('/' + tag) + i + 1;
    if (indexFinish === -1) {
      childrenTags[tag] = '';
      i = i + 1;
      continue;
    ...
    var subArray = tempArray.slice(i + 1, indexFinish);
...
module.exports = function getDomTree(template) {
  var reg = /<(\w+)[^>]*>|<(\/(\w+)[^>]*)>/g;
  var Tags = getTags(template, reg);

  reg = /<(\w+)\s*([^>]+)*>|<(\/(\w+)[^>]*)>/g;
  var Attributes = getAttributes(template, reg);
